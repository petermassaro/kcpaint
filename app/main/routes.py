...
def edit(quoteId):
	...
	if form.validate_on_submit():
		...
		return redirect(url_for('main.quoteRequests', statusFlag=False))
	return render_template('quote.html', form=form)


@bp.route('/quoteRequests/Active=<statusFlag>')
@login_required
@admin_required
def quoteRequests(statusFlag):
	statusFlag = True if statusFlag == 'True' else False
	page = request.args.get('page', 1, type=int)
	requests = QuoteRequests.query.filter_by(complete=statusFlag).order_by(QuoteRequests.submit_time.desc()).paginate(
		page, current_app.config['QRS_PER_PAGE'], False)
	next_url = url_for('main.quoteRequests', statusFlag=statusFlag, page=requests.next_num) \
		if requests.has_next else None
	prev_url = url_for('main.quoteRequests', statusFlag=statusFlag, page=requests.prev_num) \
...
def toggleStatus(quoteId):
	q_request = QuoteRequests.query.filter_by(id=quoteId).first_or_404()
	if q_request.complete:
		q_request.complete = False
	else:
		q_request.complete = True
	db.session.commit()
	return redirect(url_for('main.quoteRequests', statusFlag=False))
